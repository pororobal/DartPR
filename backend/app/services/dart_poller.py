...
def _guess_category(title: str, raw_text: str) -> str:
    ...
    """
    combined = (title + " " + raw_text[:2000]).upper()

    # Delisting risk (will be caught by hard-fail but just in case)
    if any(kw in combined for kw in ["감사의견", "횡령", "배임", "감자", "상장폐지"]):
        return "DELISTING_RISK"

    # Biotech
    if any(kw in combined for kw in ["임상", "FDA", "EMA", "IND", "NDA", "기술이전",
                                       "품목허가", "바이오", "신약"]):
        return "BIOTECH"

    # Capital raising
    if any(kw in combined for kw in ["유상증자", "CB 발행", "BW 발행", "전환사채",
                                       "신주인수권", "주주배정", "3자배정"]):
        return "CAPITAL_RAISING"

    # Business contract
    if any(kw in combined for kw in ["공급계약", "판매계약", "수주", "무상증자",
                                       "단일판매"]):
        return "BUSINESS_CONTRACT"

    # Earnings
    if any(kw in combined for kw in ["실적", "영업이익", "매출액", "당기순이익",
                                       "흑자전환", "적자"]):
        return "EARNINGS"

    # Shareholder return
    if any(kw in combined for kw in ["자사주", "취득", "소각", "최대주주변경",
                                       "배당", "주주환원"]):
